...
const ACTION_TYPE_FORBIDDEN = 'forbidden';
 ...
 * @param {String} url
 */
function showResult(data, msg = '', code = CODE.CODE_SUCCESS, action = ACTION_TYPE_SUCCESS, url = '') {
    return {
        code,
        action,
        data,
 ...
 * @param {*} action
 */
function showError(errorMsg = '', errorCode = CODE.SYSTEM_ERROR, data = {}, action = ACTION_TYPE_ALERT) {
    let url = ''
 ...
 * @param {*} errorNo
 */
function redirectTo(url = '', msg = '', data = {}, errorNo = CODE.URL_REDIRECT) {
    let action = ACTION_TYPE_REDIRECT
    return showResult(data, msg, errorNo, action, url)
}

function needLoginIn(msg = '请先登录') {
    return showResult({}, msg, CODE.NOT_LOGIN, ACTION_TYPE_LOGIN, '/login');
}

function noPrivilege(msg = '没有权限') {
    return showResult({}, msg, CODE.NO_FORBIDDEN, ACTION_TYPE_FORBIDDEN, '')
}
